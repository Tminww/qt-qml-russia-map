    ...
    for (const QJsonValue &polygon : coordinates) {
        ...
        for (const QJsonValue &ring : polygonArray) {
            ...
            for (const QJsonValue &coord : ringArray) {
                if (!coord.isArray()) continue;
                
                QJsonArray coordArray = coord.toArray();
                if (coordArray.size() < 2) continue;
                ...
                if (first) {
                    pathCommands.append(QString("M %1 %2").arg(normalizedX, 0, 'f', 2)
                                                           .arg(normalizedY, 0, 'f', 2));
                    first = false;
                } else {
                    pathCommands.append(QString("L %1 %2").arg(normalizedX, 0, 'f', 2)
                                                           .arg(normalizedY, 0, 'f', 2));
                }
            }
...
void MapData::createTestData()
{
    ...
    QVariantList testRegions = {
        QVariantMap{
            {"name", "Москва и Московская область"},
            {"id", "moscow"},
            {"paths", QVariantList{"M 300 200 L 350 200 L 350 250 L 300 250 Z"}}
        },
        QVariantMap{
            {"name", "Санкт-Петербург"},
            {"id", "spb"},
            {"paths", QVariantList{"M 250 100 L 300 100 L 300 150 L 250 150 Z"}}
        },
        QVariantMap{
            {"name", "Краснодарский край"},
            {"id", "krasnodar"},
            {"paths", QVariantList{"M 250 400 L 300 400 L 300 450 L 250 450 Z"}}
        },
        QVariantMap{
            {"name", "Свердловская область"},
            {"id", "sverdlovsk"},
            {"paths", QVariantList{"M 450 200 L 500 200 L 500 250 L 450 250 Z"}}
        },
        QVariantMap{
            {"name", "Новосибирская область"},
            {"id", "novosibirsk"},
            {"paths", QVariantList{"M 550 250 L 600 250 L 600 300 L 550 300 Z"}}
        }
    };
    
    for (const QVariant &region : testRegions) {
        m_regions.append(region);
        QVariantMap regionMap = region.toMap();
        QString id = regionMap["id"].toString();
        m_regionColors[id] = "#e0e0e0";
    }
    
    qDebug() << "Создано тестовых регионов:" << m_regions.size();
    emit regionsChanged();
    emit regionColorsChanged();
}